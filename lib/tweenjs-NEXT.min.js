...
e));if("guide"!=d)return c;var f;c.hasOwnProperty("path")||(c.path=[]);d=c.path;c.hasOwnProperty("end")||(c.end=1);c.hasOwnProperty("start")||(c.start=h&&h.hasOwnProperty("end")&&h.path===d?h.end:0);if(c.hasOwnProperty("_segments")&&c._length)return c;h=d.length;if(6<=h&&0==(h-2)%4){c._segments=[];c._length=0;for(var g=2;g<h;g+=4){for(var j=d[g-2],l=d[g-1],k=d[g+0],p=d[g+1],x=d[g+2],y=d[g+3],v=j,w=l,s,m,r=0,t=[],u=1;10>=u;u++){m=u/10;var q=1-m;s=q*q*j+2*q*m*k+m*m*x;m=q*q*l+2*q*m*p+m*m*y;r+=t[t.push(Math.sqrt((f=
